<template>
  <div class="add-question">
    <h1 class="questions-title">{{ title }}</h1>
...
export default class QuestionForm extends Vue {
  @Prop({ type: Object, default: () => ({}) }) data!: QuestionItemData;
  @Prop({ type: String, default: "Редактирование вопроса" }) title!: string;
  ...
  private async mounted() {
    ...
    if (questionId) {
      ...
      const question = this.getQuestions.find((q) => q._id === questionId);
      if (question) {
        this.fillDataFromObject(question);
      }
    } else {
      this.fillDataFromObject(this.data);
    }
  }

  fillDataFromObject(data: QuestionItemData): void {
    const {
      text,
      theme,
      lang,
  ...
  onChangeFileInput(e: InputEvent): void {
